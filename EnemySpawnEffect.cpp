...
EnemySpawnEffect::EnemySpawnEffect(float scale)
{
	model = ResourceManager::Instance().LoadModelResource("Data/Fbx/StEnemySpawnEffect/StEnemySpawnEffect.fbx");

	//--- < 頂点シェーダーオブジェクトと入力レイアウトオブジェクトの生成 > ---
	const char* csoName{ "./Data/Shader/EnemySpawnEffect_VS.cso" };
	...
	//--- < ピクセルシェーダーオブジェクトの生成 > ---
	csoName = { "./Data/Shader/EnemySpawnEffect_PS.cso" };
...
void EnemySpawnEffect::Render()
{
	...
	model->Render(transform, { 1,1,1,nowAlpha }, NULL);
